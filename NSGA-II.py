...
def init_pop(pop_size,num_obj,obj_weights,normalize,num_DV,min_vals,max_vals):
    ...
    decision_vals = np.random.uniform(low=min_vals, high=max_vals, size=(pop_size,num_DV))
...
def eval_pop(decision_vars,num_obj,obj_weights,normalize):
    ...
    for i in range(pop_size): 
        obj_functions[i,:] = eval_obj(decision_vars[i,:],num_obj,obj_weights,normalize)
...
def sort_pop(P,num_obj):
    ...
    for i in range(pop_size):
        ...
        for j in range(pop_size):
            ...
            elif p is not q and sum(p[num_DV:total_dims]) > sum(q[num_DV:total_dims]):
                n[i] +=1   
    ...
    for i in range(len(F)):
        for j in reversed(range(len(F))):
            ...
            if i is not j:
                ...
                elif (rank[i] is rank[j] and dists[i] > dists[j]) and (F[i] not in F_new):
                    F_new.append(F[i])
                elif (rank[i] is rank[j] and dists[j] > dists[i]) and (F[j] not in F_new):
                    F_new.append(F[j])
...
def reproduction(parents,num_obj,obj_weights,normalize,num_DV,min_vals,max_vals,sbx_prob,sbx_param,mutat_prob,mutat_param):
    ...
    while c < pop_size:
        ...
        if a < sbx_prob: 
            ...
            for v in range(num_DV):
                child1_vals.append(0.5*(((1+beta)*parent1_vals[v])+((1-beta)*parent2_vals[v])))
                child2_vals.append(0.5*(((1-beta)*parent1_vals[v])+((1+beta)*parent2_vals[v])))
...
def nsga(pop_size,num_gens,num_obj,obj_weights,num_DV,min_vals,max_vals,normalize,sbx_prob,sbx_param,mutat_prob,mutat_param):
    ...
    P_init = init_pop(pop_size,num_obj,obj_weights,normalize,num_DV,min_vals,max_vals)
    P = P_init
    ...
    while t < num_gens:
        ...
        Q = reproduction(P_new,num_obj,obj_weights,normalize,num_DV,min_vals,max_vals,sbx_prob,sbx_param,mutat_prob,mutat_param)
